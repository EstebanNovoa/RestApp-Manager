...
export function TextWHeader(prop){
    ...
    return(
        <div>
            ...
            <div className="w-full  border-solid border-b-2 border-black text-center"><input className="mt-10  font-cuprum text-2xl  text-gray-400 " id={prop.id} value= {prop.text} readOnly></input></div></div>
    );
}
class Prop{
    type;
...
export function NumberWithHeaderInput(prop){
    ...
    return(
        <div>
            <a className="text-3xl font-cuprum text-main-blue">{prop.header}</a><br/><br/>
            <div className="w-full  border-solid border-b-2 border-black text-center mt-10"><a className="mt-10 font-cuprum text-2xl  text-main-blue text-center" >{prop.text + " "} <input type={prop.type} id={prop.id}  className=" outline-none text-center hover:non" min="2" max="9" step="1" defaultValue='2' ></input></a></div>
            </div>
    );
}
